...
PathPlan::PathPlan(const SwathRecorder &last_swath, BoatSide side, BPolygon op_region, double margin,
                   double max_bend_angle, bool restrict_to_region)
    : m_restrict_asv_to_region(restrict_to_region),
      m_max_bend_angle(max_bend_angle),
      m_margin(margin),
      m_op_region{op_region},
      m_last_line(last_swath),
      m_planning_side(side)
{
...
PathList PathPlan::GenerateNextPath()
{
  ...
  for (size_t i = 0; i < edge_pts.size(); i++)
  {
    ...
    if (i > 0)
    {
      back_vec = EPoint(edge_pts[i].x() - edge_pts[i - 1].x(), edge_pts[i].y() - edge_pts[i - 1].y());
      back_vec_set = true;
    }
    if (i < edge_pts.size() - 1)
    {
      forward_vec = EPoint(edge_pts[i + 1].x() - edge_pts[i].x(), edge_pts[i + 1].y() - edge_pts[i].y());
    }
    else
    {
    ...
    if (back_vec.squaredNorm() != 0 && forward_vec.squaredNorm() != 0)
    {
      ...
      //       ROS_DEBUG_STREAM_COND(DEBUG,"Swath Width: %0.2f  Offset X: %0.2f Offset Y: %0.2f Avg Vec: <%0.2f,
      //       %0.2f>\n",
...
void PathPlan::RemoveAll(std::function<void(std::list<Eigen::Vector2d> &)> process, PathList &path_points)
{
...
void PathPlan::RemoveIntersects(std::list<EPoint> &path_pts)
{
  ...
  while (path_iter != last_test_seg && path_iter != back_3 && path_iter != back_2)
  {
    // Segment to test
...
void PathPlan::RemoveBends(std::list<EPoint> &path_pts)
{
  ...
  while (next_seg[1] < last_index)
  {
    ...
    if (VectorAngle(this_vec, next_vec) > m_max_bend_angle)
    {
      ...
      {
        EPoint test2_vec1 = VectorFromSegment(v_pts, SegIndex{*(std::prev(non_bend_idx.end(), 3)), this_seg[0]});
        ...
        {
          if (pts_elim1 > pts_elim2 && pts_elim1 < (pts_elim2 * 2) && angle1 < angle2)
          {
            ROS_DEBUG_COND(DEBUG,
                           "Bend Fudging - pts_elim1: %ld, pts_elim2: %ld\n\t"
                           "angle1: %.2f, angle2: %.2f\n",
                           pts_elim1, pts_elim2, angle1, angle2);

            pts_elim2 = pts_elim1 + 1;
          }
          else if (pts_elim2 >= pts_elim1 && pts_elim2 < (pts_elim1 * 2) && angle2 < angle1)
          {
            ROS_DEBUG_COND(DEBUG,
                           "Bend Fudging - pts_elim1: %ld, pts_elim2: %ld\n\t"
                           "angle1: %.2f, angle2: %.2f\n",
      ...
      while (fwd_angle > m_max_bend_angle || back_angle > m_max_bend_angle)
      {
        ...
        EPoint btwn_vec = VectorFromSegment(v_pts, SegIndex{test_back[1], test_fwd[0]});
  ...
  {
    SegIndex first_vec{*(std::prev(non_bend_idx.end(), 3)), *(std::prev(non_bend_idx.end(), 2))};
    SegIndex second_vec{*(std::prev(non_bend_idx.end(), 2)), *(std::prev(non_bend_idx.end(), 1))};
    double end_angle = VectorAngle(VectorFromSegment(v_pts, first_vec), VectorFromSegment(v_pts, second_vec));
...
std::pair<bool, bool> PathPlan::ClipToRegion2(std::list<EPoint> &path_pts)
{
  ...
  for (auto point = std::next(first_point); point != path_pts.end(); point++)
  {
    ...
    if (!boost::geometry::within(last_pt, outer_ring) && point_inside)
    {
  ...
  {
    // find last point outside
    for (auto point = std::prev(end_point); point != path_pts.begin(); point--)
    {
      BPoint this_pt(point->x(), point->y());
      // If transistion from out to inside
      if (!boost::geometry::within(last_pt, outer_ring) && boost::geometry::within(this_pt, outer_ring))
      {
        last_outside = last_pt;
        begin_inside = point;
        last_inside = this_pt;
  ...
  while (point != path_pts.end())
  {
    ...
    else if (clip_mode == 2 && !point_inside)
    {
      ...
      if (count_in > 2)
      {
        ...
        if (intersect_pts.size() > 0)
        {
          insert_out = path_pts.insert(point, EPointFromBPoint(intersect_pts[intersect_pts.size() - 1]));
...
void PathPlan::ExtendToEdge(std::list<EPoint> &path_points, bool begin)
{
  ...
  std::pair<double, EPoint> intersection = FindNearestIntersect(extend_vec, starting_pt, m_op_region);
...
std::pair<double, EPoint> PathPlan::FindNearestIntersect(EPoint ray_vector, EPoint start_pt, BPolygon &poly)
{
  ...
  for (auto poly_vertex = ext_ring.begin(); poly_vertex != std::prev(ext_ring.end()); ++poly_vertex)
  {
    auto next_vertex = std::next(poly_vertex);
    std::pair<BPoint, BPoint> poly_seg = std::make_pair(*poly_vertex, *next_vertex);
    std::pair<bool, EPoint> intersection = IntersectRaySegment(ray_vector, start_pt, poly_seg);
  ...
  auto min_index = std::min_element(intersect_dist.begin(), intersect_dist.end()) - intersect_dist.begin();
                                                      ...
                                                      std::pair<BPoint, BPoint> segment)
{
  ...
  EPoint seg_vector = EPointFromBPoint(segment.second) - EPointFromBPoint(segment.first);
